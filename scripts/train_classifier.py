...
def main():
    ...
    Parses CLI options (data root, model choice, training hyperparameters, and runtime settings), seeds randomness, constructs a SonicsConfig and SonicsDataset, splits the dataset into training and validation sets according to the validation fraction, and builds DataLoaders. It selects and wraps the chosen model in the RealFakeLitModule, configures TensorBoard logging and a ModelCheckpoint that monitors "val/eer", and runs Trainer.fit with mixed-precision and automatic accelerator/device selection.
    """
